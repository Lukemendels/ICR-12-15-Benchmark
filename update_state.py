...
for claim in claims:
 old=previous_by_key.get((claim.get('icr_id'),claim.get('evidence')))
 if old:
  for key in ['locator','evidence_anchors','verification_status']:
   if key in old:claim[key]=old[key]
...
state=json.loads((R/'research/research-state.json').read_text());state.update({'mission_status':state.get('mission_status','IN_PROGRESS'),'current_iteration':max(d['review_iteration'] for d in reviews),'icrs_reviewed':len(reviews),'agencies_components_reviewed':sorted(set(d['agency'] for d in reviews)),'current_leaders':{'best_individual_recent':[d['icr_id'] for d in reviews if d['total_score']==best['total_score']],'by_dimension':{k:[d['icr_id'] for d in reviews if d['score'][k]==max(r['score'][k] for r in reviews)] for k in reviews[0]['score']},'agency_leader':'insufficient repeated evidence','most_consistent_agency':'not estimable'},'current_tsa_benchmark':{'n':len(tsa),'mean':statistics.mean(d['total_score'] for d in tsa),'min':min(d['total_score'] for d in tsa),'max':max(d['total_score'] for d in tsa),'conclusion':'Observed TSA sample shows useful methods and material reconstruction gaps; agency-wide ranking and overhaul effects remain unestablished.'},'saturation_streak':state.get('saturation_streak',0),'unresolved_evidence_gaps':state.get('unresolved_evidence_gaps',[]),'next_research_strategy':state.get('next_research_strategy','See latest iteration log'),'updated_at':datetime.datetime.now(datetime.timezone.utc).isoformat()});(R/'research/research-state.json').write_text(json.dumps(state,indent=2))
