...
The recent-package window is defined by actual receipt dates, September 9, 2021, through September 9, 2026, rather than by the date-like characters in a package reference number. The archived official search produced 92 rows: 90 TSA ICR packages and two OPM common-form requests excluded from TSA-authored statement comparison. The inventory also identifies 48 assigned TSA control histories and five packages with unassigned control numbers. A blank control number does not create a new independent collection identity. [[INVENTORY]]
...
Other stable components include an eight-hour LEOSA rejection-review request, a 30-minute maximum for pipeline physical-incident reporting, a one-hour DASSP initial operator application coefficient, claim filing, a 130-second Secure Flight identity-transmission component, and a 10-minute canine adoption application. The incident-reporting statistic is a maximum, not a measured mean. DASSP field scope is not proven identical across versions, and neighboring armed-security-officer nomination work is excluded. Secure Flight resolution calls and visitor applications are outside the identity-transmission comparison. [[CMP-26,CMP-27,CMP-29,CMP-30,CMP-31,CMP-32]]
